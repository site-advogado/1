...
function startTimer(duration) {
  ...
  _contadorInterval = setInterval(() => {
    ...
    if (remaining <= 0) {
      clearInterval(_contadorInterval);
      if (labelEl) labelEl.textContent = 'CÓDIGO EXPIRADO';
      else if (timerEl) timerEl.textContent = 'EXPIRADO';
      return;
    }
...
async function checarNotificacao(email) {
  ...
  try {
    const res  = await fetch(URL_API, { // URL_API vem de tema.js
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (searchInput) {
    searchInput.addEventListener('blur', () => {
      checarNotificacao(searchInput.value.trim());
    });
